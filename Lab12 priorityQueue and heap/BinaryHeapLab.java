...
class BinaryHeapLab
{
   ...
   //O(logn)
    ...
    {
    	int tmp = heap.get(childInd);
    	while(childInd > 0 && tmp < heap.get(parent(childInd))){
    		//heap.add(childInd, heap.get(parent(childInd)));
    		int valParent = heap.get(parent(childInd));
    		heap.set(childInd, valParent);
    		childInd = parent(childInd); 	
    	}
 
    	//heap.add(childInd, tmp); //? childInd is index and tmp is value?
    ...
    {
    	System.out.println("Couting time");
    	ArrayList<Integer> i = BinaryHeapLab.createInput(100000, 1000);
    	BinaryHeapLab bin = new BinaryHeapLab();
    	int j;
    	...
    	while(bin.heap.size() != 0){
    		bin.deleteMin();
    	}
    	System.out.println((System.nanoTime()-time)/1000000000.0);
    	
    	/* n			insert			remove
    	 * 12500		0.012297875		0.036778098
    	 * 25000		0.021845734		0.089387399
    	 * 50000		0.021971281		0.246225654
    	 * 100000		0.034811481		1.037865693
    	 */
